...
func main() {
	...
	s.HandleFunc("GET", "/users/:user_id/addresses/:address_id", func(c *Context) {
		...
		c.RenderJson(u)
		// fmt.Fprintf(c.ResponseWriter, "retirev user %v's address %v\n", c.Params["user_id"], c.Params["address_id"])
	})
	...
	s.HandleFunc("POST", "/login", func(c *Context) {
		...
		// id와 passoword 가 맞지 않으면 다시 "/login" 페이지 렌더링
	...
	/*
		...
		r.HandleFunc("GET", "/users/:id", logHandler(recoverHandler(func(c *Context) {
			...
			fmt.Fprintf(c.ResponseWriter, "retrive user %v\n", c.Params["id"])
		})))

		r.HandleFunc("GET", "/users/:user_id/addresses/:address_id", func(c *Context) {
			fmt.Fprintf(c.ResponseWriter, "retrive user %v 's address %v\n", c.Params["user_id"], c.Params["address_id"])
		})
